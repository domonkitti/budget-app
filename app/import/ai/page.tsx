...
import type { AIImportPreviewResult, AIImportApplyResult, AIImportCompareRow, AIImportYearTotal } from "@/lib/types"
...
function ComparisonTable({ rows }: { rows: AIImportCompareRow[] }) {
  ...
  const grand = rows.reduce((acc, r) => ({
    oldCommitted: acc.oldCommitted + r.old_budget_committed,
    oldInvest: acc.oldInvest + r.old_budget_invest,
    newCommitted: acc.newCommitted + r.new_budget_committed,
    newInvest: acc.newInvest + r.new_budget_invest,
  }), { oldCommitted: 0, oldInvest: 0, newCommitted: 0, newInvest: 0 })
  const grandOldTotal = grand.oldCommitted + grand.oldInvest
  const grandNewTotal = grand.newCommitted + grand.newInvest
  const grandDCommitted = grand.newCommitted - grand.oldCommitted
  const grandDInvest = grand.newInvest - grand.oldInvest
  ...
  return (
    <div className="mb-6 bg-white rounded-xl border p-4">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">เปรียบเทียบก่อน/หลังนำเข้า</h3>
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="border-b text-gray-500">
              ...
              <th className="text-center py-1 px-2 font-medium text-xs border-l" colSpan={3}>วงเงินดำเนินการ (เดิม)</th>
              <th className="text-center py-1 px-2 font-medium text-xs border-l" colSpan={3}>วงเงินดำเนินการ (ใหม่)</th>
          ...
          <tbody>
            {rows.map((r) => {
              const oldTotal = r.old_budget_committed + r.old_budget_invest
              const newTotal = r.new_budget_committed + r.new_budget_invest
              const dCommitted = r.new_budget_committed - r.old_budget_committed
              const dInvest = r.new_budget_invest - r.old_budget_invest
              ...
              return (
                <tr key={r.project_type} className="border-b border-gray-50">
                  ...
                  <td className={`${tdNum} border-l text-gray-500`}>{fmtM(r.old_budget_committed)}</td>
                  <td className={`${tdNum} text-gray-500`}>{fmtM(r.old_budget_invest)}</td>
                  <td className={`${tdNum} text-gray-600 font-medium`}>{fmtM(oldTotal)}</td>
                  <td className={`${tdNum} border-l text-blue-500`}>{fmtM(r.new_budget_committed)}</td>
                  <td className={`${tdNum} text-blue-500`}>{fmtM(r.new_budget_invest)}</td>
                  <td className={`${tdNum} text-blue-600 font-medium`}>{fmtM(newTotal)}</td>
                  <td className={`${tdNum} border-l font-medium ${dColor(dCommitted)}`}>{dStr(dCommitted)}</td>
                  <td className={`${tdNum} font-medium ${dColor(dInvest)}`}>{dStr(dInvest)}</td>
            ...
            <tr className="bg-gray-100 border-t">
              ...
              <td className={`${tdNum} border-l font-bold text-gray-700`}>{fmtM(grand.oldCommitted)}</td>
              <td className={`${tdNum} font-bold text-gray-700`}>{fmtM(grand.oldInvest)}</td>
              <td className={`${tdNum} font-bold text-gray-700`}>{fmtM(grandOldTotal)}</td>
              <td className={`${tdNum} border-l font-bold text-blue-700`}>{fmtM(grand.newCommitted)}</td>
              <td className={`${tdNum} font-bold text-blue-700`}>{fmtM(grand.newInvest)}</td>
              <td className={`${tdNum} font-bold text-blue-700`}>{fmtM(grandNewTotal)}</td>
              <td className={`${tdNum} border-l font-bold ${dColor(grandDCommitted)}`}>{dStr(grandDCommitted)}</td>
              <td className={`${tdNum} font-bold ${dColor(grandDInvest)}`}>{dStr(grandDInvest)}</td>
...
export default function AIImportPage() {
  ...
  function resetDownstream() {
    setPreview(null); setForcedNew(new Set()); setResult(null); setError(null); setSnapshotted(false)
  }
  ...
  async function handleCheck() {
    ...
    setError(null); setResult(null); setForcedNew(new Set())
    setChecking(true)
    try {
  ...
  async function handleImport() {
    ...
    const matchedCount = preview.items.filter((it) => it.matched_code && !forcedNew.has(it.row_key)).length
    const newCount = preview.items.length - matchedCount
    const warning = snapshotted
      ? ""
      : "\n\n⚠️ ยังไม่ได้สร้าง snapshot — แนะนำให้สร้างก่อน เผื่อต้องย้อนกลับ"
    if (!confirm(
      `อัปเดต ${matchedCount} โครงการที่ตรงกัน และสร้างใหม่ ${newCount} โครงการ จาก ${loaded.length} ไฟล์?${warning}`
    )) return
    setImporting(true)
    setError(null)
    try {
      const payload = { ...mergeBatches(loaded), force_new_row_keys: [...forcedNew] }
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {preview && !result && (
        <>
          ...
          <ComparisonTable rows={preview.comparison} />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {preview.items.map((it) => {
                  ...
                  const isMatched = !!it.matched_code && !isForcedNew
                  ...
                  return (
                    <tr key={it.row_key} className="border-t">
                      ...
                      <td className="px-3 py-2 max-w-md truncate">{it.name}</td>
